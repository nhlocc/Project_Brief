...
class NumberGuessingGame():
    ...
    def start_game(self):
        ...
        """
        try:
            self.logger.info(f"Welcome {self.player_name} to Number Guessing Game!"
                             f" You are having {self.player_points} points\n")
            while self.LOSE_THRESHOLD < self.player_points <= self.WIN_THRESHOLD:
                user_input = utilities.user_input("Are you ready to start a new match? (Yes/No)",
                                                  r'\b(yes|y|no|n)\b')
            ...
            if self.player_points >= self.WIN_THRESHOLD:
                ...
                self.logger.info(
                    "Player stopped playing the game with {} points\n".format(self.player_points))
    ...
    def start_match(self, player_points: int):
        ...
        try:
            ...
            round_number = 1
            reward_points, round_number = self.start_round(self.reward_points, round_number)
            while reward_points > 0:
                user_input = utilities.user_input(
                    f'Current player points: {player_points} - Current reward: {reward_points}.'
    ...
    def start_round(self, reward_points: int, round_number: int):
        ...
        try:
            ...
            if re.search(fr'\b({player_guess})\b', result):
                ...
                self.logger.info("Your card and House'card are equal in this round\n")
            else:
                self.logger.info("Player loses\n")
                reward_points = 0
            self.logger.info(
                f"Your current reward point after this round {round_number} is: {reward_points}\n")
            round_number += 1
